...
class _FollowState extends State<Follow> {
  ...
  Widget wordsCard(Article article) {
    Widget markWidget;
    if (article.imgUrl == null) { //如果文章不附加图片就只显示文字
      markWidget = new Text(
        article.mark,
          ...
          children: <Widget>[
            new Container(
              child: new Row(
                children: <Widget>[
                  new Container(
                    child: new CircleAvatar(  //头像小圈
                        backgroundImage: new NetworkImage(article.headUrl),
                        radius: 11.0
                    ),
                  ),
                  new Text("  " + article.user , style: new TextStyle(color: GlobalConfig.fontColor))
